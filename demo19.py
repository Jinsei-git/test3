...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        if st.session_state.last_uploaded_file_info != current_file_info:
                    ...
                    for _, row in df.iterrows():
                        try:
                            ...
                            if "genre" not in event or event["genre"] not in st.session_state.genres:
                                event["genre"] = "その他"
                                if "その他" not in st.session_state.genres:
                            ...
                            st.warning(f"CSVの行の処理中にエラーが発生しました（一部スキップ）: {e}")
                            continue
                        
                    if new_events:
                        # 既存イベントに追加する方式に変更（上書きではなく）
                        st.session_state.events.extend(new_events)
                        
                        # 新しいジャンルに対するカラーマッピング更新
                        for event in new_events:
                            if event["genre"] not in st.session_state.color_map:
                                available_colors = [color for color in FIXED_COLOR_PALETTE if color not in st.session_state.color_map.values()]
                                if available_colors:
                                    st.session_state.color_map[event["genre"]] = available_colors[0]
                                else:
                                    st.session_state.color_map[event["genre"]] = FIXED_COLOR_PALETTE[len(st.session_state.color_map) % len(FIXED_COLOR_PALETTE)]
                        
                        # 処理済みファイル情報を保存
                        st.session_state.last_uploaded_file_info = current_file_info
                        
                        st.success(f"CSVから{len(new_events)}件のイベントを追加しました。")
                        st.rerun()
                    else:
            ...
            st.info("このファイルは既に処理済みです。")
    
    # ファイルが削除された時の情報初期化
    if not uploaded_file and st.session_state.last_uploaded_file_info:
        st.session_state.last_uploaded_file_info = None
